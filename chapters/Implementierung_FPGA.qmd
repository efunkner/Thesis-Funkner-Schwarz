...
Das Top-Level Modul bildet die oberste Instanz und bindet alle Funktionseinheiten zusammen. Die AXI4-Stream Schnittstellen werden von zwei Modulen beschreiben, je eines für den AXI4-Stream Slave und Master. Der AXI4-Stream Slave empfängt die Eingangsdaten von einem übergeordneten System, welche zunächst in einem Eingangs-FIFO zwischengespeichert werden. Die eigentliche Signalverarbeitung findet im Design-Under-Test (DUT) statt. ier werden die gepufferten Eingangsdaten durch eine Kaskade von Biquad-Filtersektionen geleitet. Unter dem DUT ist ein Modul, welches den Gesamt-Biquad-Filter beschreibt, welchser selbst mehere Sectionen enthalten kann. Jede einzelne Seczion wird durch ein eigenes Modul implementiert.  Für die Filterung wird das Signal durch diese Sectionen geführt, bis die die gewünschte Filtercharakteristik erreicht ist.<br>
...
Die Ausgabe erfolgt über den AXI4-Stream Master, der die gefilterten Daten wieder an das übergeordnete System liefert. in separates Modul zur Reset-Synchronisation sorgt dafür, dass ein globales Reset-Signal sicher in allen Takt- und Logikbereichen übernommen wird.<br>

Für den Datenfluss ergibt sich damit: Die Eingangsdaten werden über AXI4-Stream empfangen, in FIFOs gepuffert, durch den Biquad-Filter verarbeitet, wieder gepuffert und schließlich über AXI4-Stream ausgegeben. Dabei wird der Datenfluss über Handshake-Signale gesteuert, um Datenverlust oder Überläufe zu vermeiden.

Die gesamte Struktur ist klar hierarchisch aufgebaut. Das Top-Level Modul bindet alle Funktionseinheiten, as DUT (HP_Butter_2_1kHz_fix32_16_v2_dut.vhd) enthält die Kernverarbeitung, während separate Module für die Filterstufen, Pufferung (FIFOs), Speicher (RAM) und Schnittstellen (AXI4-Stream) zuständig sind.<br>

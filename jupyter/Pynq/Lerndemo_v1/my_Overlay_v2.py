...
# Plottet einfaches Spektrum über fft
def plot_simple_spectrum(frames, Fs, fft_size=2048, start=0):
    colors = ['blue', 'purple']
    labels = ['Kanal 0 (links)', 'Kanal 1 (rechts)']
    plt.figure(figsize=(12, 4))

    for channel in range(min(2, frames.shape[1])):
        signal = frames[start:start + fft_size, channel]
        signal = signal - np.mean(signal)

        spectrum = np.abs(fft(signal))[:fft_size // 2]
        spectrum_db = 20 * np.log10(spectrum / np.max(spectrum) + 1e-12)
        freqs = np.fft.fftfreq(fft_size, d=1/Fs)[:fft_size // 2]

        plt.fill_between(freqs, spectrum_db, y2=-80, color=colors[channel], alpha=0.6, label=labels[channel])
    ...
    plt.title("Frequenzspektrum beider Kanäle")
